...
from utils.io import read_txt_to_struct, read_txt_to_struct_and_ajdust_fromat, read_seqmaps, \
...
def evaluate_sequence(trackDB, gtDB, distractor_ids, iou_thres=0.5, minvis=-2): #shai, minvis=0
    """
    ...
    """
    trackDB, gtDB = preprocessingDB(trackDB, gtDB, distractor_ids, iou_thres, minvis) # BEN
...
def create_files(path, label_dir, pred_dir):  # CHANGE IT BEN ! MAKE 1 FILE FOR ALL GT
    write_res_path = os.path.join(path, 'res1.txt')
    resDB = []
    write_gt_path = os.path.join(path, 'gt1.txt')
    gtDB = []

    for i, (wp, db, rp) in enumerate(zip((write_res_path, write_gt_path), (resDB, gtDB), (pred_dir, label_dir))):
        files =[f for f in glob.glob(rp+'**\*.txt', recursive=True)]
        # read = read_txt_to_struct_and_ajdust_fromat
        # if i == 0:
        read = read_txt_to_struct
        for f in files:
            db.append(read(f))
        with open(wp, 'wb') as f:
            np.savetxt(f, np.concatenate(db, axis=0) , delimiter=', ')



def evaluate_tracking(path, output_path):
    all_info = []
    track_res = os.path.join(path, 'res1.txt')
    gt_file = os.path.join(path, 'gt1.txt')
    ...
    print_metrics('Summary Evaluation', all_metrics, output_path)
...
def parse_args():
    ...
    parser.add_argument('--seqmap', default='seqmaps/test.txt' , help='seqmap file', type=str)
    parser.add_argument('--path',default='data/', help='Tracking result directory', type=str)
    parser.add_argument('--output_path',default='data/metric_results.txt', help='write metric results path', type=str)
    parser.add_argument('--labels-dir', default='Z:\\yolo_14_09\\data\\Baloons_DATA\\retagged_7_8_22\\Tracking_Dataset\\From4Cams_0\\target_1_seq_0', help='gt labels result directory', type=str)
    parser.add_argument('--pred-dir', default='..\\..\\runs\\track\\tracking_with_MOT_format_baloons_From4Cams_0_seq0_shai\\bytetracker', help='Tracking result directory', type=str)
...
if __name__ == '__main__':
    args = parse_args()
    create_files(args.path, args.labels_dir, args.pred_dir)
    evaluate_tracking(args.path, args.output_path)
