...
def read_txt_to_struct_and_ajdust_fromat(fname):
    """
    read txt to structure, and change to column represents:
    [frame number] [identity number] [bbox left] [bbox top]
     [bbox width] [bbox height] [DET: detection score,
      GT: ignored class flag] [class] [visibility ratio]
    ...
    with open(fname, 'r') as fid:
        lines = fid.readlines()
        for line in lines:
            line = list(map(float, line.strip().split(', '))) # (frame_idx + 1, c, id, bbox_left, bbox_top, bbox_w, bbox_h, -1, -1, -1, -1, conf))
            c = line[1]
            conf = line[-1]
            line = line[:1] + line[2:8] + [c] + [-1.,-1.,-1.] + [conf] #(frame_idx + 1, id, bbox_left, bbox_top, bbox_w, bbox_h, -1, c, -1, -1, -1, conf) (BYTESORT output)
            data.append(line)
    data = np.array(data)
    # change point-size format to two-points format
    data[:, 4:6] += data[:, 2:4]
...
def extract_valid_gt_data(all_data, remove_ofv=False):
    ...
    """
    distractor_classes = [] # EVERYONE EXCEPT SWIMMER
    valid_classes = [0,1,3,4,5,6,7,8,9,10,2]  # SWIMMER
    original = all_data.shape[0]
    # remove classes in other classes, pedestrain and distractors
    # left for furthur usages
    selected = np.array([i for i in range(all_data.shape[0]) if all_data[i, 7] in valid_classes + distractor_classes]).astype(np.int)
    ...
    #  filtering
    if remove_ofv:
        selected = np.array([i for i in range(all_data.shape[0]) if (all_data[i, 2] + all_data[i, 4]) / 2 >= 0 and
...
def print_metrics(header, metrics, output_path=None, banner=25):
    if len(metrics) == 17:
