...
def detect(opt):
    ...
    if webcam:
        ...
        try:
            cam_source = Path(source).read_text().rsplit()[0]
        except:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                    ...
                    for _, t in enumerate(online_targets):
                        ...
                        for one_det in det:
                            ...
                            if round(float(one_conf),2) == round(float(score),2) and int(one_clss) == int(cls):
                                x1, y1 ,x2, y2 = one_det[0:4].cpu()
                        online.append(np.array([x1, y1, x2, y2, tid, cls, score]))
                        # ORATRDetection contains: TrackId (Int64), Conf (UInt16), Classification (UInt16), FrameCoordULX (UInt16), FrameCoordULY (UInt16), FrameCoordBRX (UInt16), FrameCoordBRY (UInt16)
                        res[i].append({'TrackId': np.int(tid), 'Conf': np.int(score*100), 'Classification':np.int(cls), 'FrameCoordULX':np.int(x1.item()), 'FrameCoordULY':np.int(y1.item()), 'FrameCoordBRX':np.int(x2.item()), 'FrameCoordBRY':np.int(y2.item())}) # like ORATRDetection object
                    if len(online) > 0:
                ...
                if len(outputs[i]) > 0:
                    ...
                    # print('res [i]: %s' % res[i])
                    # print('json res [i]: %s' % json.dumps(res[i]))
                    porject_swimmers = True
                    print(res[i])
                    if porject_swimmers:
                        try:  # SEND message per camera's  !!!
                            print('Sending message from camera index : %s' % cam_source)
                            url = "http://localhost:8000/send_results/"
                            headers = {"Content-type": "application/json", "Accept": "application/json"}
                            # print('len of res: %s' % len(res[i]))
                            data = {
                                'CamId': int(cam_source),  # Uint16
                                'FrameId': frame_idx,  # Uint16
                                'NumDetections': len(res[i]),
                                'AlgoType': 0,
                                'ATRDetections': res[i]  # Type ORATRDetection
                                # res[i] = [tid, score, cls, x1, y1, x2, y2]
                                # ORATRDetection contains: TrackId (Int64), Conf (UInt16), Classification (UInt16), FrameCoordULX (UInt16), FrameCoordULY (UInt16), FrameCoordBRX (UInt16), FrameCoordBRY (UInt16)
                            }
                ...
                try:  # SEND message per camera's  !!!
                    print('Sending an EMPTY message from camera index : %s' % cam_source)
                    url = "http://localhost:8000/send_results/"
                    headers = {"Content-type": "application/json", "Accept": "application/json"}
                    # print('len of res: %s' % len(res[i]))
                    data = {
            ...
            # Baloons
            #cls_name = ['Drone', 'AirPlane', 'UFO', 'AirPlane_w_lights', 'Baloons', 'Drone_w_lights',
            #            'Single_front_lights', 'other']
            # Swimmers      0                 1                 2           3                4            5
            cls_name = ['Person', 'Person on board vessel', 'Swimmer', 'Sail boat', 'Floating Object', 'Dvora',
                        #  6       7           8             9                10
                        'Zeara', 'PWC', 'Marchant Ship','Inflatable Boat', 'Vessel']
            ...
            if opt.add_yolobboxes:  # det[:,0:4]= xyxy , det[:, 4]= confg , det[:, 5]= class
                for val in det:
                    ...
                    cv2.putText(im0, cls_name[cls] + '_' + str(conf), (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, 2)
