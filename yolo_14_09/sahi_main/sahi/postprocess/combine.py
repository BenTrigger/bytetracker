...
from sahi_main.sahi.annotation import Mask, BoundingBox, Category
import numpy as np
import copy
...
class PostprocessPredictions:
    ...
    def get_score_func(object_prediction: ObjectPrediction):
        ...
        return object_prediction['score']#.score.value

    @staticmethod
    def has_same_category_id(pred1: ObjectPrediction, pred2: ObjectPrediction) -> bool:
        return pred1['category'] == pred2['category'] #pred1.category.id == pred2.category.id
    ...
    def calculate_bbox_iou(pred1: ObjectPrediction, pred2: ObjectPrediction) -> float:
        ...
        box1 = pred1['bbox'][0] #np.array(pred1.bbox.to_voc_bbox())
        box2 = pred2['bbox'][0] #np.array(pred2.bbox.to_voc_bbox())
