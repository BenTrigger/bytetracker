...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK in [-1, 0]:
            ...
            if (not nosave) or (final_epoch and not evolve):  # if save
                ...
                last = str(Path(last).parents[0] / ('last_' + str(epoch) + '.pt')) # BEN save weights every epoch.
                best = str(Path(best).parents[0] / ('best_' + str(epoch) + '.pt')) # BEN save weights every epoch.
    ...
    if RANK in [-1, 0]:
        ...
        if not evolve:
            ...
            # Strip optimizers
            for f in last, best:
                print('check f param: %s' % f)
                if Path(f).exists():  # Ben - changed f to Path(f)
...
def main(opt):
    ...
    # Evolve hyperparameters (optional)
        ...
        print("HERE_CHECK_EVOLVE_OUT")
        for _ in range(opt.evolve):  # generations to evolve
            print("HERE_CHECK_EVOLVE_IN_1")
            if Path('evolve.txt').exists():  # if evolve.txt exists: select best hyps and mutate
                print("HERE_CHECK_EVOLVE_IN_2")
                # Select parent(s)
        ...
        # Plot results
        print("HERE_CHECK_EVOLVE_IN_1.1")
        plot_evolution(yaml_file)
        print("HERE_CHECK_EVOLVE_IN_1.2")
