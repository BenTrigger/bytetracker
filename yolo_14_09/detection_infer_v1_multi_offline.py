...
class YoloDetection:
    def __init__(self, weights='', cuda=''):
        self.weights = weights  # 'model.pt path(s)'
        self.source = ''  # input data src
        self.imgsz = 1024  # inference size (pixels)
    ...
    def run_inference(self, starting_pixels, imgs):
        # Run inference
        self.img_obj = True
        t0 = time.time()
        self.webcam = False
        ...
        for batch in batches:
            ...
            for n in range(preds.shape[0]):
                ...
                pred = non_max_suppression(pred, self.conf_thres, self.iou_thres, classes=[8], agnostic=False)
                img = gimgs[counter+n]
                # Process detections
                slice_preds=[]
                    ...
                    if self.webcam:  # batch_size >= 1
                        ...
                        try:
                            p, s, im0, frame = f'{starting_pixels[counter+n][0]}_{starting_pixels[counter+n][1]}_' \
                                               f'{starting_pixels[counter+n][0] + 1024}_{starting_pixels[counter+n][0] + 1024}', \
                    ...
                    if len(det):
                        ...
                        for *xyxy, conf, cls in reversed(det):
                            ...
                            if conf > self.conf_thres:
                                ...
                                slice_preds.append((xmin, ymin, xmax, ymax))
                sliced_predictions.append(slice_preds)
            counter += batch

        return predictions, sliced_predictions


if __name__ == '__main__':
    t = time.time()
    yv5 = YoloDetection(weights='/data2/mritime/sahi/best.pt', cuda='', )
    print(f'Done. ({time.time() - t:.3f}s)')
    t = time.time()

    yv5.load_dataset('/data2/mritime/sahi/slices/a/')
    a = yv5.run_inference()
    print(f'Done. ({time.time() - t:.3f}s)')

    t = time.time()

    yv5.load_dataset('/data2/mritime/sahi/slices/images/')
    b = yv5.run_inference()
    print(f'Done. ({time.time() - t:.3f}s)')
