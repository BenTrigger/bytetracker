...
class LoadImages:  # for inference
    def __init__(self, path, img_size=640, stride=32):
        ...
        #     print("here2")
        #     p += '/*'
        #     folders = sorted(glob.glob(p, recursive=True))  # glob
        #     print(folders)
        #     for dir in folders:
...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640, stride=32): # BEN for zafrir project img_size=3840
        self.mode = 'stream'
        self.img_size = img_size
        self.stride = stride
        ...
        for i, s in enumerate(sources):  # index, source
            ...
            # try:
            #     cv2.imshow('test', cap.read())
            # except Exception as e:
            #     print(e)
            #cap.set(cv2.CAP_PROP_FRAME_WIDTH,3840)  # BEN for zafrir project
            #cap.set(cv2.CAP_PROP_FRAME_HEIGHT,2160) # BEN for zafrir project
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        # BEN RUN IT WITH agument param = True and uncomment slef.albumentations
        try:
            f = []  # image files
    ...
    def __getitem__(self, index):
        ...
        if self.augment:
            #print("self.augment: %s" % self.augment) # SHAI ZODEK: its True
            # Albumentations
            img, labels = self.albumentations(img, labels)
            # if random.random() < 0.01: #grayscale
            #     img = T.Grayscale()(img)
            # if random.random() < 0.1: #blur
            #     img = T.GaussianBlur(kernel_sizes=(30,30))(img)
            # HSV color-space
        ...
        img = img.transpose((2, 0, 1))[::-1]  # HWC to CHW, BGR to RGB
        img = np.ascontiguousarray(img)
        # t_img = T.ToTensor()(img) #shai, standartization
        t_img =  torch.from_numpy(img)
        return t_img, labels_out, self.img_files[index], shapes
