...
class Tracker:
    ...
    def update(self, detections, classes, confidences):
        ...
        matches, unmatched_tracks, unmatched_detections = \
            self._match_ben(detections) # self._match(detections)
    ...
    def _full_cost_metric(self, tracks, dets, track_indices, detection_indices):
        ...
        # print(f'Min Pos Matrix ={np.min(pos_cost)}, mean {np.mean(pos_cost)}, max {np.max(pos_cost)}') #shai
        pos_gate = pos_cost > 1.0
        # Now Compute the Appearance-based Cost Matrix
        # print(f'feat shape: {np.array([dets[i].feature for i in detection_indices]).shape}')
        ...
        # print(f'Min COST Matrix ={np.min(app_cost)}, mean {np.mean(app_cost)}, max {np.max(app_cost)}') #shai
    ...
    def _match_ben(self, detections):
        # Split track set into confirmed and unconfirmed tracks.
        confirmed_tracks = [i for i, t in enumerate(self.tracks) if t.is_confirmed()]
        unconfirmed_tracks = [i for i, t in enumerate(self.tracks) if not t.is_confirmed()]
        # print("features")
        ...
        # # Associate remaining tracks together with unconfirmed tracks using IOU.
        iou_track_candidates = unconfirmed_tracks + [
            k for k in unmatched_tracks_a if self.tracks[k].time_since_update < iou_time
        ]
        unmatched_tracks_a = [
            k for k in unmatched_tracks_a if self.tracks[k].time_since_update >= iou_time
        ]
        matches_b, unmatched_tracks_b, unmatched_detections = linear_assignment.min_cost_matching(
            iou_matching.large_iou_cost,
            self.max_iou_distance,
            self.tracks,
            detections,
            iou_track_candidates,
