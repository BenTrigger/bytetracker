...
from gpio_siren import ActivateSiren
...
from yolov5_v7.utils.dataloaders2 import LoadImages , LoadStreams, VID_FORMATS # changed from datasets to dataloaders
...
# from pydantic import BaseModel # For Pedestal
...
def make_noise():
    global noise_not_in_progress
    print('noise_in_progress: %s'% noise_not_in_progress)
    if noise_not_in_progress:
        cmd = "gst-launch-1.0 filesrc location=Drone_record.wav ! wavparse ! audioconvert ! audioresample ! alsasink device=hw:0"
        noise_not_in_progress = False
        #x = subprocess.Popen(cmd, shell=True)
    	ActivateSiren()
        counting_time_thread = Thread(target=timer_counter)
        counting_time_thread.start()

def timer_counter():
    global noise_not_in_progress
    time.sleep(10)
    noise_not_in_progress = True
    print('end of thread changing noise_in_progress to: %s'%noise_in_progress)
...
def detect(opt):
    ...
    if webcam:
        ...
        #cudnn.benchmark = True  # set True to speed up constant image size inference
        dataset = LoadStreams(opt.nw, opt.nh, source, img_size=imgsz, stride=stride, auto=pt)
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                if len(outputs[i]) > 0:
                    ...
                    if opt.magi_alg:
                        #EK - This section asks the User whether the target is a FALSE ALARM. If it is - add it to a FALSE ALARMS list and dont engage it,
                        #  if it isnt, then engage it
                        ...
                        if (AskUser == True):
                            print (" target %s is being looped\n"%(id))
                            if( trackUser != id and (id not in FalseAlarmsList)): #it is not a known FALSE and also not tracked - so ask the user
                                User = input("Do you see target %s ? Press y if yes, and n if Not\n"%(id))
                                if (User == "y"):       
                                    print (FalseAlarmsList)
                                    print (trackUser)
                                    User = input("Is target %s a False Alarm? Press f if it is, and t if it is a target\n"%(id))
                    
                                    if(User=="f"):
                                        FalseAlarmsList.append(id)
                                        print("added to FA list\n")
                                    elif(User=="t"):
                                        trackUser = id
                                        print ("tracking the target\n")
                        else:
                            trackUser = id
                        ...
                        if(trackUser == id):
                            print("PEDESTAL COMMAND for target %s \n"%(id))
                            try:
            ...
            if opt.add_yolobboxes:  # det[:,0:4]= xyxy , det[:, 4]= confg , det[:, 5]= class
                for val in det:
                    ...
                    cv2.putText(im0, names[cls] + '_' + str(conf), (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 1.0, color, 2)
            ...
            if save_vid:
                ...
                if opt.out_video:
                    ...
                    if opt.raw_recording or (det is not None and len(det)):  # SAVE IMAGES ONLY WHILE WE HAVE DETECTIONS
                        ...
                        new_folder = Path(str(Path(save_path).parents[0] / Path(time_now)))
                        new_folder.mkdir(exist_ok=True)
                        new_path = str(new_folder) + '/' + str(file_name_counter) + '.png'
                        if det is not None and len(det):
                            #file_name_counter += 1
                            cv2.imwrite(new_path, im0)
                        
                        if opt.raw_recording:
                            recording_path = str(new_folder) + '/' + 'recording' + '/' + str(file_name_counter) + '.png'
                            Path(str(new_folder) + '/' + 'recording').mkdir(exist_ok=True)
                            cv2.imwrite(recording_path, imc1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--nw', type=int, default=3840, help='display width')  # video display width
    parser.add_argument('--source', type=str, default='0', help='source')  # file/folder, 0 for webcam
    parser.add_argument('--nh', type=int, default=2160, help='display height')  # video display width
